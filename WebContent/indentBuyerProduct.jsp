...
<script src="js/indentBuyerProduct.js"></script>
</head>
<body>
	<%
		User user = new User();
		...
		String sql = "SELECT * FROM product,indent WHERE product.pid=indent.pid AND uid='" + user.getUid() + "'";
		if (state != null && state != "") {
			sql += " AND state='" + state + "'";
		}
		sql += ";";
 ...
 %>
				...
				<td align="center">
					...
					%> <a
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=取消订单">取消订单</a>
					<%
						} else if (state.equals("已发货")) {
					%> <a
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=确认收货">确认收货</a>
					<%
						} else if (state.equals("交易成功")) {
					%> <a
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=请求退款">请求退款</a><br />
					<br /> <a onClick="return confirmDelete();"
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=删除success">删除</a>
					<%
						} else if (state.equals("退款中")) {
					%><a
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=取消退款">取消退款</a>
					<%
						} else if (state.equals("交易取消")) {
					%><a
					href="<%=request.getContextPath()%>/IndentBuyerServlet?iid=<%=rs.getString("iid")%>&op=删除cancel">删除</a>
					<%
						}
					%>
				</td>
		...
		<br> 第<%=showPage%>页（共<%=pageCount%>页） <br> <a
			href="indentBuyerShowPage.jsp?showPage=1">首页</a> <a
			href="indentBuyerShowPage.jsp?showPage=<%=showPage - 1%>">上一页</a>
		<%
			//根据pageCount的值显示每一页的数字并附加上相应的超链接
			for (int i = 1; i <= pageCount; i++) {
		%>
		<a href="indentBuyerShowPage.jsp?showPage=<%=i%>"><%=i%></a>
		<%
			}
		%>
		<a href="indentBuyerShowPage.jsp?showPage=<%=showPage + 1%>">下一页</a> <a
			href="indentBuyerShowPage.jsp?showPage=<%=pageCount%>">末页</a>
		<!-- 通过表单提交用户想要显示的页数 -->
		<form action="indentBuyerShowPage.jsp" method="post">
