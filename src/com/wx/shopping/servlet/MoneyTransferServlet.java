...
public class MoneyTransferServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			boolean ret=userDao.moneyTransfer(user.getUid(), request.getParameter("receiver"),
					request.getParameter("money"), request.getParameter("remark"));

			if(ret)
			{
				User userNew = userDao.selectByUid(user.getUid());
				request.getSession().setAttribute("loginuser", userNew);
				response.sendRedirect(request.getContextPath() + "/moneyTransferOk.jsp");
				
			}else{
				response.sendRedirect(request.getContextPath() + "/moneyTransferFail.jsp");
			}
